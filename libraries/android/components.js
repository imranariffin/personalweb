...
prx.types.android_actionbar = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		if(typeof(item.selectionIcon)!= "undefined" && item.selectionIcon.url != "") {
			cR += '<div class="left-icon" style="line-height: '+_height+'px;">'
			cR += '<img src="'+prx.componentsHelper.getAssetUrl(item.selectionIcon)+'" style="height: '+(item.selectionIconSize * 20)+'%; border-right: 1px solid '+prx.utils.getColor(item.borderColor)+'; padding-left: 10px;"/>'
			cR += '</div>'
		}
...
prx.types.android_card_big = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		cR += '<style>';
		cR += '#' + _id + ' .android-big-card-wrapper { background-color: '+prx.utils.getColor(item.backgroundColor)+'; box-shadow: 0 8px 6px -6px '+prx.utils.getColor(item.shadowColor)+'; }';
...
prx.types.android_slider = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		cR += '#'+_id+' .android-slider-button { background-color: '+prx.utils.getColor(item.fillBarColor)+'; width: '+(item.barThickness*4)+'px; height: '+(item.barThickness*4)+'px; margin-left: -'+(item.barThickness*2)+'px; margin-right: -'+(item.barThickness*2)+'px; top: '+((_dims.height/2)-((item.barThickness*4)/2))+'px; left: '+item.sliderPosition+'%; }';
	...
	,onResize: function(item,containerid,pageid,symbol) {
		...
		$('#'+_id+' .android-slider-button').css({
			top: ((_dims.height/2)-((item.barThickness*4)/2))+'px',
	...
	,afterDisplay: function(item,containerid,pageid,symbol) {
		...
		if(!prx.editor) {
			...
			Draggable.create('#'+_id+' .android-slider-button', {
				type: 'x',
				bounds: { left: -(item.barThickness*2), width: parseInt(_dims.width)+parseInt((item.barThickness*4)), top: 0, height: 0 },
